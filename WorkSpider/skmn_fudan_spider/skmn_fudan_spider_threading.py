...
def get_index_page(index_queue, crawl_queue):
    """
    :param index_queue:索引页码队列
    :param crawl_queue: 文章url队列
    """
    # 获取队列中的url直到为空
...
class ThreadCrawl(threading.Thread):
    def __init__(self, thread_name, crawl_queue, parse_queue):
        """
        初始化
        :param thread_name: 采集线程名字
        :param crawl_queue: 采集队列
        :param parse_queue: 解析队列
        """
    ...
    def run(self):
        ...
        while not CRAWL_EXIT:
            try:
                ...
                url = self.crawl_queue.get(False)
                # print(url)
                self.get_page(url)
            except:
                pass
    ...
    def get_page(self, url):
        ...
        if page_response:
            ...
            if source_article:
                ...
                list_content.append(source_article)
                self.parse_queue.put(list_content)

class ThreadParse(threading.Thread):
    def __init__(self, thread_name, parse_queue, lock):
        """
        初始化
        :param thread_name: 采集线程名字
        :param parse_queue: 解析队列
    ...
    def run(self):
        ...
        while not PARSE_EXIT:
            try:
                # 从页码队列中获取数据，False队列为空抛出一个为空的异常
                list_content = self.parse_queue.get(False)
                # print(list_content)
                self.get_page(list_content)
            except:
                pass
    ...
    def get_page(self, list_content):
        """
        :param list_content:从解析队列中获取到的list_content
        """
        url = list_content[0]
        # print(url)
        source_article = list_content[1]
        # print(source_article)
        ...
        if judge_img_get:
            ...
            list_article = self.parse_page(source_article)
            # print(list_article)
            # 保存文章内容 
            self.save_page(list_article, filename)
            # self.save_mysql(url, filename)
        else:
            print('获取不到图片：' + url)


    def parse_page(self, source_local):
        """
        提取文章内容
        ...
        source_article = html_source_local.xpath('//div[@class="view-info"]/span')[0].text
        ...
        # print('source_article',source_article)

        # 通过正则表达式获取文章中需要的内容，即正文部分
        pattren_article_content = re.compile(r'<div class="view-cnt">(.*)<div class="clearfix">', re.I|re.S)
    ...
    def save_page(self, list_article, filename):
        """
        保存到文件
    ...
    def save_mysql(self, url_source, url_local):
        """
        保存到文件
        ...
        cursor = db.cursor()
        url_local_full = sys.path[0] + url_local  
        update_time = time.strftime('%Y-%m-%d',time.localtime())
        data = {
            'source_url':url_source,
            'local_url':url_local_full,
            'source':'skmn.fudan.edu.cn',
...
def main():
    ...
    for thread_name in list_crawl:
        # 线程定义
        thread = ThreadCrawl(thread_name, crawl_queue, parse_queue)
        # 启动线程
        thread.start()
        # 添加线程到列表
    ...
    # 存储4个解析线程的列表集合 
    thread_parse = []
    for thread_name in list_parse:
        # 线程定义5
        thread = ThreadParse(thread_name, parse_queue, lock)
        # 启动线程
        thread.start()
        # 添加线程到列表
    ...
    get_index_page(index_queue, crawl_queue)

    # 等待crawl_queue队列为空
    while not crawl_queue.empty():
        pass
    # 如果crawl_queue队列为空,采集线程退出循环
    global CRAWL_EXIT
    CRAWL_EXIT = True
    print('crawl_queue为空！')
