...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        # 拼接url
        index_full_url = index_url + str(index_page) + '.htm'
        ...
        if index_response:
            ...
            index_source_list = index_html.xpath('//a[@class="news_box clearfix"]/@href')
            # print(index_source_list)
            # 写入当前爬取到的第一个文章url
            if index_page == 1 and index_source_list:
                next_judge = index_source_list[0]
            ...
            for index_source in index_source_list:
                ...
                # 获取文章部分url
                page_url = 'http://www.shanghaitech.edu.cn' + index_source
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r"<div class='wp_articlecontent'>.*<div class=\"infonext\">", re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://www.shanghaitech.edu.cn' + img_part_url[1].replace('..','')
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html('.arti_title').text().strip()
                ...
                # 获取文章更新时间
                try:
                    article_update_time = article_html('.t').text().strip()
                except:
                    article_update_time = ''
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/shanghaitech_edu_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/shanghaitech_edu_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/shanghaitech_edu_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.shanghaitech.edu.cn'
...
def main():
    ...
    遍历每一页索引页
    """
    print("shanghaitech_edu_spider爬取开始！")
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.shanghaitech.edu.cn/1006/list'
            # 要爬取的索引总页数
            index_page = 3
    ...
    print("shanghaitech_edu_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
