...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_response:
            ...
            index_source_list = index_html('.normal_list li a')
            ...
            for index_source in index_source_list.items():
                ...
                if index_source.attr('href') == last_judge:
                    ...
                    break
                if '.html' in index_source.attr('href'):
                # 获取文章部分url
                    page_url = 'http://www.bio.pku.edu.cn' + index_source.attr('href')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div class="txt">(.*)上一篇：', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://www.bio.pku.edu.cn' + img_part_url[1].replace('..','')
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html('h3').text().strip()
                ...
                # 获取文章更新时间
                try:
                    article_update_time_pattern = re.compile(r'日期：.*(\d{4}-\d{2}-\d{2})')
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/bio_pku_edu_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/bio_pku_edu_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/bio_pku_edu_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.bio.pku.edu.cn'
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.bio.pku.edu.cn/homes/Index/news/22/22.html'
            # 要爬取的索引总页数
            index_page = 2
    ...
    print("bio_pku_edu_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
