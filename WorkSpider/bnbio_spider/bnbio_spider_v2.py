...
def index_page(page, judge, judge_name, url):
    ...
    for i in range(1,page):    
        ...
        if response_index:
            ...
            source_index = html_index.xpath('//ul[@id="lib_info_list"]//a/@href')
            # 写入当前爬取到的第一个文章url
            ...
            for item in source_index:
                ...
                # item: /html_news/news_30096.html
                # print(item)
                get_page(item)
...
def get_page(url):
    ...
    :param url:文章链接需要的参数
    """
    # url: /html_news/news_30096.html
    # url_full：http://www.scib.ac.cn/xwzx/zhxw/201811/t20181119_5189195.html
    url_full = 'http://www.bnbio.com' + url
    # print(url_full)
    with open(sys.path[0] + '/user-agents.txt', 'r', encoding = 'utf-8') as f:
        list_user_agents = f.readlines()
        user_agent = random.choice(list_user_agents).strip()
    ...
    if response_article:
        # 通过正则表达式获取文章中需要的内容
        pattren_article = re.compile(r'<div class="art_con">.*相关推荐：', re.S|re.I)
        source_article = pattren_article.search(response_article.text)
        # print(source_article)
        if source_article:
            source_article = source_article.group()
            # 获取文章中所有的图片url链接: http://www.bnbio.com/UploadFile/20181249819692.jpg
            ...
            for kw in findall_img:
                # kw[1]: http://www.bnbio.com/UploadFile/20181249819692.jpg
                # 判断图片URL是否需要组合
                # print('kw[1]',kw[1])
                ...
                if judge_img:
                    ...
                    # 图片网址:url_full_img: http://www.bnbio.com/UploadFile/20181249819692.jpg
                    url_full_img = 'http://www.bnbio.com' + kw[1]
                # print('url_full_img',url_full_img)
                pattern_name_save_img = re.compile(r'.*\/(.*\.[jpbg][pmin]\w+)', re.I)
                try:
                    # 图片保存名： 20181249819692.jpg
...
def parse_page(source_local):
    ...
    title_article = html_source_local.xpath('//div[@class="newstitle"]')[0].text
    title_article = '<title>' + title_article + '</title>\n' + '</head>\n'
    list_article.append(title_article)
    # print(title_article)

    source_article = html_source_local.xpath('//div[@class="newsmeta"]')[0].text
    # print(source_article)
    try:
    ...
    result_user = ''
    try:
        pattern_result_source = re.compile(r'来源：(.*)', re.I|re.S)
        result_source = pattern_result_source.search(source_article).group(1).replace(r'&nbsp;','').replace(r'</span>','').strip()
    except:
        result_source = ''
    # print(result_source)
    ...
    pattren_article_content = re.compile(r'<div class="contents">(.*)相关推荐：', re.I|re.S)
    ...
    if source_article:
        ...
        def img_url_name(match):
            ...
            if img_real_name and match.group(1):
                ...
                img_name = '<img src="/home/bmnars/data/bnbio_spider_result/img/' + kw_img_name + '" />'
        ...
        # 剔除一些杂乱的样式
        source_local = source_local.replace('<i>','').replace('<i style="font-size: 12pt">','').replace('&nbsp;','').strip()
...
def save_img(source, filename):
    ...
    """
    dir_save_img = '/home/bmnars/data/bnbio_spider_result/img/'
...
def save_page(list_article,filename):
    ...
    """
    dir_save_page = '/home/bmnars/data/bnbio_spider_result/'
...
def save_mysql(url_source, url_local):
    ...
    cursor = db.cursor()
    url_local_full =  '/home/bmnars/data/bnbio_spider_result/' + url_local  
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            url_kw = 'http://www.bnbio.com/nlist/nlist_2'
            num = 8
