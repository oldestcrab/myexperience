...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_response:
            ...
            index_source_list = index_html.xpath('//h2[@class="fs16 fw1 ffw"]/a/@href')
            ...
            for index_source in index_source_list:
                ...
                # 获取文章部分url
                page_url = 'http://www.nibs.ac.cn/' + index_source
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div class="info_txt fs14">.*<div class="footer jz ffw" style="border: none;">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://www.nibs.ac.cn' + img_part_url[1]
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html.xpath('//h1[@class="fs18 fw1 ffw"]')[0].text.strip()
                ...
                # 获取文章更新时间
                try:
                    article_update_html = article_html.xpath('//div[@class="time ffw jz"]')[0].text.strip()
                    article_update_time_pattern = re.compile(r'(\d{4})\/(\d{2})\/(\d{2})')
                    article_update_time_part = article_update_time_pattern.search(article_update_html)
                    article_update_time = article_update_time_part.group(1) + '-' + article_update_time_part.group(2) + '-' + article_update_time_part.group(3)
                except:
                    article_update_time = ''
...
# 图片替换路径
IMG_CHANGE_DIR = './img/'
# IMG_CHANGE_DIR = '/home/bmnars/data/nibs_ac_spider_result/img/'

# 文件存储路径
PAGE_SAVE_DIR = sys.path[0] + '/nibs_ac_spider_result/'
# PAGE_SAVE_DIR = '/home/bmnars/data/nibs_ac_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.nibs.ac.cn'
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.nibs.ac.cn/news.php?cid=4&sid=15&page='
            # 要爬取的索引总页数
            index_page = 3
