...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_response:
            ...
            index_source_list = index_html.xpath('//div[@class="news_txt"]')
            ...
            for index_source in index_source_list:
                ...
                # 获取文章部分url
                page_url = 'http://www.cscb.org.cn/' + page_part_url
...
def get_article_page(page_url, article_update_time):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'</h4>.*<div style="margin-top:40px;text-align:right;">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://www.cscb.org.cn/' + img_part_url[1]
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html.xpath('//h4')[0].text.strip()
                ...
                # 获取文章来源
                try:
                    article_origin_pattern = re.compile(r'来源：(.*?)</span>')
...
# 图片替换路径
IMG_CHANGE_DIR = './img/'
# IMG_CHANGE_DIR = '/home/bmnars/data/cscb_org_spider_result/img/'

# 文件存储路径
PAGE_SAVE_DIR = sys.path[0] + '/cscb_org_spider_result/'
# PAGE_SAVE_DIR = '/home/bmnars/data/cscb_org_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.cscb.org.cn'
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.cscb.org.cn/news_list/3/1.html'
            # 要爬取的索引总页数
            index_page = 2
    ...
    print("cscb_org_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
