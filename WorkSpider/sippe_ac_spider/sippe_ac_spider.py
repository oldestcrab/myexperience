...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_page == 1:
            ...
            index_full_url = index_url + '_' + str(index_page-1) + '.html'
        ...
        if index_response:
            ...
            index_source_list = index_html.xpath('//div[@class="media"]//div[@class="media_txt"]/a[1]/@href')
            ...
            for index_source in index_source_list:
                ...
                # 获取文章部分url
                page_url = 'http://www.sippe.ac.cn/kyjz' + index_source.replace('./', '/')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'</p>.*<div id="footer">', re.S|re.I)
        ...
        if article_source:
            ...
            try:
                ...
                img_url_kw = 'http://www.sippe.ac.cn'
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html.xpath('//p[@class="text-center title"]')[0].text.strip()
...
# 图片替换路径
IMG_CHANGE_DIR = './img/'
# IMG_CHANGE_DIR = '/home/bmnars/data/sippe_ac_spider_result/img/'

# 文件存储路径
PAGE_SAVE_DIR = sys.path[0] + '/sippe_ac_spider_result/'
# PAGE_SAVE_DIR = '/home/bmnars/data/sippe_ac_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.sippe.ac.cn/'
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.sippe.ac.cn/kyjz/index'
            # 要爬取的索引总页数
            index_page = 4
    ...
    print("sippe_ac_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
