...
def get_page(url, url_kw):
    ...
    if source_article:
        ...
        def url_img_name(match):
            ...
            """
            # ./W020180828369497938690.jpg
            pattren_img_local = re.compile(r'.[pjbg][pinm]')
            img_real_name = pattren_img_local.search(match.group())

            if img_real_name:
                img_name  = ' src="/home/bmnars/data/ibp_cas_spider_result/img/' + match.group(1).replace('./','') + '"'
                return img_name
...
def save_img(source, filename):
    ...
    """
    name_save_img = '/home/bmnars/data/ibp_cas_spider_result/img/' + filename 
    try:
        # 保存图片
...
def save_page(source,filename):
    ...
    """
    try:
        with open('/home/bmnars/data/ibp_cas_spider_result/' + filename, 'w', encoding = 'utf-8') as f:
            f.write(source)
    except  OSError as e:
...
def save_mysql(url_source, url_local):
    ...
    cursor = db.cursor()
    url_local_full = '/home/bmnars/data/ibp_cas_spider_result/' + url_local 
