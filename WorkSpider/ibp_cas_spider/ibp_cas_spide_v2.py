...
def get_page(url, url_kw):
    ...
    try:
        ...
        print('获取不到文章：' + url_full)
        response_article = requests.get('https://www.baidu.com', headers = headers)
    # 通过正则表达式获取文章中需要的内容
...
def save_img(source, filename):
    ...
    """
    dir_save_img = sys.path[0] + '/ibp_cas_spider_result/img/'
...
def save_page(list_article,filename):
    ...
    """
    dir_save_page = sys.path[0] + '/ibp_cas_spider_result/'
