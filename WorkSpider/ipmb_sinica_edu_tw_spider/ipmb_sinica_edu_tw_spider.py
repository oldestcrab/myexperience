...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        with open(sys.path[0] + '/sld.html', 'w', encoding = 'utf-8') as f:
            f.write(index_response.text)
        if index_response:
            # 通过xpath获取索引页内的文章列表url
            index_html = etree.HTML(index_response.text)
            index_source_list = index_html.xpath('//div[@class="event"]/span/a/@href')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div class="col-md-12 ">(.*)<div class="otherinfo">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'https://ipmb.sinica.edu.tw' + img_part_url[1].replace('..','')
            ...
            if IMG_EXISTS:
                ...
                # 获取文章更新时间
                try:
                    article_update_time_pattern = re.compile(r'發稿時間:(\d{4}-\d{2}-\d{2})')
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/ipmb_sinica_edu_tw_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/ipmb_sinica_edu_tw_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/ipmb_sinica_edu_tw_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'https://ipmb.sinica.edu.tw'
...
def main():
    ...
    遍历每一页索引页
    """
    print("ipmb_sinica_edu_tw_spider爬取开始！")
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'https://ipmb.sinica.edu.tw/ch/activities/highlights'
            # 要爬取的索引总页数
            index_page = 2
    ...
    print("ipmb_sinica_edu_tw_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
