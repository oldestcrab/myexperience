...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_response:
            ...
            index_source_list = index_html('.txt a')
            ...
            for index_source in index_source_list.items():
                ...
                if index_source.attr('href') == last_judge:
                    ...
                    break
                # 获取文章部分url
                page_url = 'http://ibs.fudan.edu.cn/' + index_source.attr('href')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div id="www_zzjs_net" style="font-size:14px;line-height:24px;">(.*)<div class="clear">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://ibs.fudan.edu.cn' + img_part_url[1].replace('..','')
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html.xpath('//strong[@class="bt_hui"]')[0].text.strip()
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/ibs_fudan_edu_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/ibs_fudan_edu_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/ibs_fudan_edu_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://ibs.fudan.edu.cn'
...
def main():
    ...
    遍历每一页索引页
    """
    print("ibs_fudan_edu_spider爬取开始！")
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://ibs.fudan.edu.cn/research_2.php?year=2018'
            # 要爬取的索引总页数
            index_page = 2
    ...
    print("ibs_fudan_edu_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
