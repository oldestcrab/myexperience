...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        if index_response:
            ...
            index_source_list = index_html.xpath('//div[@class="span2"]/a/@href')
            ...
            for index_source in index_source_list:
                ...
                if index_source == last_judge:
                    ...
                    break
                # 获取文章部分url
                page_url = 'http://icob.sinica.edu.tw/' + index_source
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<td align="right">(.*)<div  class="container">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://icob.sinica.edu.tw' + img_part_url[1].replace('..','')
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/icob_sinica_edu_tw_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/icob_sinica_edu_tw_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/icob_sinica_edu_tw_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://icob.sinica.edu.tw'
...
def main():
    ...
    遍历每一页索引页
    """
    print("icob_sinica_edu_tw_spider爬取开始！")
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://icob.sinica.edu.tw/list.php?subtype=%E7%A0%94%E7%A9%B6%E6%88%90%E6%9E%9C'
            # 要爬取的索引总页数
            index_page = 2
    ...
    print("icob_sinica_edu_tw_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
