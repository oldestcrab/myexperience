...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        # 拼接url
        if index_page == 1:
            index_full_url = index_url + '.html'
        else:
            index_full_url = index_url + '_' + str(index_page) + '.html'
        ...
        if index_response:
            ...
            index_source_list = index_html('.withimg a')
            ...
            for index_source in index_source_list.items():
                ...
                if index_source.attr('href') == last_judge:
                    ...
                    break
                # 获取文章部分url
                page_url = 'http://life.tsinghua.edu.cn' + index_source.attr('href')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div class="article">(.*)<div class="footer">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://life.tsinghua.edu.cn' + img_part_url[1].replace('..','')
            ...
            if IMG_EXISTS:
                ...
                # 获取文章标题
                article_title = article_html.xpath('//div[@class="title"]/h1')[0].text.strip()
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/life_tsinghua_edu_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/life_tsinghua_edu_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/life_tsinghua_edu_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://life.tsinghua.edu.cn'
...
def main():
    ...
    遍历每一页索引页
    """
    print("life_tsinghua_edu_spider爬取开始！")
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://life.tsinghua.edu.cn/publish/smkx/11191/index'
            # 要爬取的索引总页数
            index_page = 4
    ...
    print("life_tsinghua_edu_spider爬取完毕，脚本退出！")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
