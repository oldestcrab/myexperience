...
def get_index_page(index_page, last_judge, last_judge_name, index_url):
    ...
    for index_page in range(1,index_page):    
        ...
        # 拼接url
        index_full_url = index_url + '?p=' + str(index_page)
        ...
        if index_response:
            ...
            index_source_list = index_html('h3 a')
            ...
            for index_source in index_source_list.items():
                ...
                # 获取文章部分url
                page_url = 'http://www.pumch.cn' + index_source.attr('href')
...
def get_article_page(page_url):
    ...
    if article_response:
        # 通过正则表达式获取文章中需要的内容
        article_pattren = re.compile(r'<div class="text">.*<div class="share_details">', re.S|re.I)
        ...
        if article_source:
            ...
            for img_part_url in img_url_list:
                ...
                if img_judge:
                    ...
                    img_url = 'http://www.pumch.cn' + img_part_url[1]
                # print('img_url: ',img_url)

                img_save_name_pattern = re.compile(r'.*\/(.*\.[jpbg][pmin]\w+)', re.I)
                try:
                    # 图片保存名
                    img_save_name = img_save_name_pattern.search(img_part_url[1]).group(1).replace(r'/','').replace(r'\\','').replace(':','').replace('*','').replace('"','').replace('<','').replace('>','').replace('|','').replace('?','').replace('%','').strip()
            ...
            if IMG_EXISTS:
                ...
                filename = filename.replace(r'/','').replace(r'\\','').replace(':','').replace('*','').replace('"','').replace('<','').replace('>','').replace('|','').replace('?','').strip()
                ...
                # 获取文章标题
                article_title = article_html('.main div.t').text()
                # print('article_title', article_title)
                # 获取文章作者
                try:
                    article_user = article_html('.tip span:nth-child(5) ').text().replace('作者：','')
                except:
                    article_user = ''
                # print('article_user', article_user)
                # 获取文章更新时间
                try:
                    article_update_time = article_html('.tip i').text().replace('.','-')
                except:
                    article_update_time = ''
...
if run_as == 1:
    ...
    IMG_CHANGE_DIR = './img/'
    # 文件存储路径
    PAGE_SAVE_DIR = sys.path[0] + '/pumch_spider_result/'
else:
    IMG_CHANGE_DIR = '/home/bmnars/data/pumch_spider_result/img/'
    PAGE_SAVE_DIR = '/home/bmnars/data/pumch_spider_result/'

# 文章来源网站
ARTICLE_ORIGIN_WEBSITE = 'http://www.pumch.cn'
...
def main():
    ...
    for wd in range(1):
        ...
        if wd == 0:
            ...
            # 索引url
            index_url = 'http://www.pumch.cn/research_dynamic.html'
            # 要爬取的索引总页数
            index_page = 3
